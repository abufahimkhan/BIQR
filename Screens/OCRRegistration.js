...
export default function OCRLicence() {
  ...
  const handleTextUpload = () => {
    // console.log('Edited Text:', editedText);
     const id_R = route.params.id; // Retrieve the docId from navigation params
    firestore()
      .collection('USERS')
      .doc(id_R) // Use the docId passed from the previous screen
      .update({Registration_Info: editedText})
      .then(() => {
  ...
  const handleNextButton = () => {
    if (uploadDone) {
      navigation.navigate('ocr_taxtoken',{id_R:route.params.id});
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentInsetAdjustmentBehavior="automatic">
        <View style={styles.content}>
          <Text style={styles.title}>OCR REGISTRATION</Text>
